...
rho_b_seq <- seq(0.4, 0.8, 0.1)         # target correlation between scales
rho_w <- 0.6                          # target correlation within scales
R <- 20                                # number of simulation runs
seed <- 20230111                      # seed of the random number generator
scale_estimator <- "Qn12"          # type of scale estimator used
...
results_list_rho <- parallel::mclapply(rho_b_seq, function(rho_b) {
    ...
    results_r <- lapply(epsilons, function(epsilon) {
      ...
      for (i in 1:(p*num_scales)) {
        scale_estimate[i, i] <- 
          ...
          robustbase::Qn(data[, i], k = choose(n,2)/2)
      }
      ...
      df_pearson_norm <- tryCatch({
        if(det(scale_estimate) == 0) {
          ...
          data.frame(Run = r, rho_b = rho_b, epsilon = epsilon, Method = "Pearson",
                     Norm = pearson_norm)
        }
      ...
      df_trans_kendall_norm <- tryCatch({
        if(det(scale_estimate) == 0) {
          ...
          data.frame(Run = r, rho_b = rho_b, epsilon = epsilon, Method = "Transformed Kendall",
                     Norm = kendall_norm)
        }
      }, error = function(e) NULL, warning = function(w) NULL)
...
file_results <- "pearson_vs_kendall/results/Qn/Rhob/results_n=%d-scales=%d-p=%d-est=%s.RData"
save(results_rho, n, p, num_scales, prob, rho_w, rho_b_seq, seed, file = sprintf(file_results, n, num_scales, p, scale_estimator))
...
aggregated <- results_rho %>%
  group_by(epsilon, Method, rho_b) %>%
  summarize(Norm = mean(Norm),
            .groups = "drop")

# plot average results over the simulation runs
library("ggplot2")
p_line <- ggplot(aggregated, aes(x = rho_b, y = Norm, color = Method)) +
  geom_line() +
  ...
  labs(title = sprintf("Frobesius norm of variance error matrix using %s", scale_estimator), subtitle = sprintf("n =% d correlation within scales = %f", n, rho_w), caption = sprintf("number of scales = %d, number of items per scale = %d", num_scales, p))

# save plot to file
file_plot <- "pearson_vs_kendall/figures/Qn/Rhob/results_n=%d-scales=%d-p=%d-estimator=%s.pdf"
