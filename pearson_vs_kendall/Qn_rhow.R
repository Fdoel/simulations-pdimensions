...
results_list_rho <- parallel::mclapply(rho_w_seq, function(rho_w) {
  ...
  results_r <- lapply(epsilons, function(epsilon) {
    ...
    Qn_estimate <- robustbase::Qn(data)
    # compute Frobenius norm of the estimation error matrix for Pearson
    df_pearson_norm <- tryCatch({
      if(Qn_estimate == 0) {
        NULL
      } else {
      # get the transformed Kendall matrix and multiply it with Qn to get the scatter matrix
      pearson_matrix <- sqrt(Qn_estimate)*cor(data, method = "pearson")*sqrt(Qn_estimate)
      # get the Frobenius norm of the error matrix
      pearson_norm <- norm(Rho - pearson_matrix, type = "F")
    ...
    df_trans_kendall_norm <- tryCatch({
      ...
      # get the transformed Kendall matrix and multiply it with Qn to get the scatter matrix
      kendall_matrix <- sqrt(Qn_estimate)*sin(pi*0.5*cor(data, method = "kendall")*sqrt(Qn_estimate))
      # get the Frobenius norm of the error matrix
      kendall_norm <- norm(Rho - kendall_matrix, type = "F")
